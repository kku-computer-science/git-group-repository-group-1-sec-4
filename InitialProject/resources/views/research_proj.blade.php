...
<div class="container refund">
    <p>{{ trans('message.projecttitle') }}</p>
                ...
                <tr>
                    ...
                    <td>
                        <div style="padding-bottom: 10px">
                            ...
                            <span style="padding-left: 10px;">
                                {{ \Carbon\Carbon::parse($re->project_start)->thaidate('j F Y') }} {{ trans('message.to') }} {{ \Carbon\Carbon::parse($re->project_end)->thaidate('j F Y') }}
                            </span>
                            @else
                            <span style="font-weight: bold;">
                                {{ trans('message.project_duration') }}
                    ...
                    @endif -->
                        ...
                        <div style="padding-bottom: 10px;">
                            ...
                            <span style="padding-left: 10px;"> @if(is_null($re->fund)) @else {{$re->fund->fund_type}} @endif</span>
                        </div>
                        <div style="padding-bottom: 10px;">
                            <span style="font-weight: bold;">{{ trans('message.supporting_agency') }}</span>
                            <span style="padding-left: 10px;"> @if(is_null($re->fund)) @else {{$re->fund->support_resource}} @endif</span>
                        </div>
                        <div style="padding-bottom: 10px;">
                        ...
                        <div style="padding-bottom: 10px;">
                            ...
                            <span style="padding-left: 10px;"> {{number_format($re->budget)}} {{ trans('message.baht') }}</span>
                        </div>
                    </td>
                    ...
                    <td style="vertical-align: top;text-align: left;">
                        <div style="padding-bottom: 10px;">
                            <span>@foreach($re->user as $user)
                                {{$user->{'position_'.app()->getLocale()} ?? $user->position }} {{$user->{'fname_'.app()->getLocale()} ?? $user->fname}} {{$user->{'lname_'.app()->getLocale()} ?? $user->lname}}<br>
                                @endforeach</span>
                        </div>
                    </td>
                    @if($re->status == 1)
                    ...
                    @elseif($re->status == 2)
                    <td style="vertical-align: top;text-align: left;">
                        <h6><label class="badge bg-warning text-dark">{{ trans('message.progress') }}</label></h6>
                    </td>
                    @else
                    <td style="vertical-align: top;text-align: left;">
